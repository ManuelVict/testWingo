  ...
  it('1) case search fly BOG to CLO with ', () => {
    ...
    const checkDisponibility = (x, y) => {
      cy.get('#inputOutboundDate').then($datePicker => {
        const dateOne = cy.get(` #inputOutboundDate > .cont-calendar > .cont-months > .date-picker-wrapper >
         :nth-child(1) > :nth-child(1) > .month-wrapper > .month1 > tbody > :nth-child( ${x}) > :nth-child( ${y}) > .day`)
    ...
    const checkDisponibilityReturn = (x, y) => {
      cy.get('#inputOutboundDate').then($datePicker => {
        const dateTwo = cy.get(`#date-window-container > .date-picker-wrapper > :nth-child(1) > :nth-child(1) > 
        .month-wrapper > .month2 > tbody > :nth-child(${x}) > :nth-child(${y}) > .day`).should('not.have.class', 'invalid');
        if (!dateTwo) {
          x = x + 1;
          y = y + 1;
